 ...
 * @author rossetti
 */
open class DBA_SingleQueueStation(parent: ModelElement?, resource: SResource? = null, sd: GetValueIfc? = ConstantRV.ZERO, sender: SendQObjectIfc? = null, name: String? = null, ) : Station(parent, name) {
    ...
    protected fun getServiceTime(stub: STUB): Double {
        ...
        t = if (useQObjectServiceTimeOption) {
            ...
            serviceTime!!.value.absoluteValue * newAdds * nr_attributes
    ...
    internal inner class EndServiceAction : EventActionIfc<STUB?> {
        override fun action(event: JSLEvent<STUB?>) {
            val leavingCustomer = event.message

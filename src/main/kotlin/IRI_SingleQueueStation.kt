 ...
 * @author rossetti
 */
open class IRI_SingleQueueStation(parent: ModelElement?, resource: SResource? = null, sd: GetValueIfc? = ConstantRV.ZERO, sender: SendQObjectIfc? = null, name: String? = null, ) : Station(parent, name) {
    ...
    protected fun getServiceTime(ean: EAN): Double {
        val t: Double
        ...
        t = if (useQObjectServiceTimeOption) {
            ...
            if (ean.completionState == 1) {
                (myServiceTime!!.value * ean.nr_attributes)
            } else {
                myServiceTime!!.value
            }
    ...
    protected fun serveNext() {
        val ean: EAN = waitingQ.removeNext() //remove the next customer
        myResource!!.seize()
        // schedule end of service
        scheduleEvent(myEndServiceAction, getServiceTime(ean), ean)
    }
    ...
    internal inner class EndServiceAction : EventActionIfc<EAN?> {
        override fun action(event: JSLEvent<EAN?>) {
            val ean = event.message
